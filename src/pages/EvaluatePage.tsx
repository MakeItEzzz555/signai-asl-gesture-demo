...
import { getModelSnapshot, subscribeModel } from '../ml/model';
...
export default function EvaluatePage() {
  const { metrics: evaluationMetrics, ready: isModelTrained, validation } = useSyncExternalStore(subscribeModel, getModelSnapshot);
      ...
      <div className="max-w-6xl space-y-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Train a model to produce a validation holdout. Loading a saved model does not restore unrelated evaluation results.
          </p>
        </div>
        <div className="bg-card border border-border rounded-xl p-12 flex flex-col items-center justify-center text-center">
          ...
          <Link href="/train" className="flex items-center gap-2 px-5 py-2.5 rounded-lg bg-primary text-primary-foreground text-sm font-semibold hover:opacity-90 transition-opacity">
  ...
  return (
    <div className="max-w-6xl space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Validation holdout for the last completed model. Samples are excluded from fitting and are not augmented.
        </p>
      </div>

      {/* ── Summary Cards ─────────────────────────────────────────── */}
      {validation && <p className="text-sm text-muted-foreground" data-testid="validation-summary">Validation holdout: {validation.sampleCount} original samples; training: {validation.trainingSampleCount} originals. Split seed: {validation.seed}. Sample-level validation does not measure unseen signers or recording sessions. Synthetic starter samples are for demonstration only.</p>}
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
        <MetricCard
          label="Accuracy"
          value={`${(accuracy * 100).toFixed(1)}%`}
          color="text-primary"
          description="Correct predictions on validation originals"
        />
        <MetricCard
